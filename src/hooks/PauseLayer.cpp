...
class $modify(ThumbnailPauseLayer, PauseLayer) {
    ...
    void onScreenshot(CCObject*) {
        ...
        ThumbnailPopup::create(levelID, saveDir, buildSubmissionNote())->show();
    }

    static int getLevelRating(GJGameLevel* level) {
        if (!level) return 0;
    ...
    static double getAccurateProgress(PlayLayer* pl) {
        return std::clamp(
    ...
    static std::string buildSubmissionNote() {
        auto playLayer = PlayLayer::get();
