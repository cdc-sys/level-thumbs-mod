...
//             .chain([levelID, filename, load](Result<std::string>* res) -> web::WebTask {
//                 if (!res->isOk()) {
...
//                 if (!readRes) {
//                     Notification::create(readRes.unwrapErr(), NotificationIcon::Error)->show();
//                     return web::WebTask::immediate(web::WebResponse());
//                 }
//
...
//                 uploadReq.body(std::move(readRes).unwrap());
//
//                 return uploadReq.post(fmt::format("{}/upload/{}", Settings::thumbnailAPIBaseURL(), levelID));
//             }, "LT Upload Task 1/2")
//             .chain([load](web::WebResponse* res) -> UploadTask {
//                 load->fadeOut();
...
//     auto readRes = file::readBinary(filename);
//     if (!readRes) {
//         if (ui) Notification::create(readRes.unwrapErr(), NotificationIcon::Error)->show();
//         return UploadTask::immediate(Err(std::move(readRes).unwrapErr()));
//     }
//
//     auto uploadReq = web::WebRequest();
//     auto token = Mod::get()->getSavedValue<std::string>("token");
...
//     if (!this->isLoggedIn()){
//         return AuthManager::login()
//             .chain([linkSecret = std::move(linkSecret)](Result<std::string>* res) mutable -> web::WebTask {
//                 if (!res->isOk()) {
//                     Notification::create(res->err().value(),NotificationIcon::Error)->show();
//                     return web::WebTask::immediate(web::WebResponse());
//                 }
...
//                 auto body = matjson::makeObject({{"token", std::move(linkSecret)}});
...
//
//         linkReq.header("Authorization",fmt::format("Bearer {}",token));
//         linkReq.bodyJSON(matjson::makeObject({{"token", std::move(linkSecret)}}));
//
//         return linkReq.post(fmt::format("{}/auth/link",Settings::thumbnailAPIBaseURL()))
//             .chain([load](web::WebResponse* res) -> LinkTask {
//                 load->fadeOut();
...
//         .chain([accID, userID = userID.value(), username = std::move(username)](geode::Result<std::string>* res) mutable -> web::WebTask {
//             if (!res->isOk()) {
...
//
//             loginReq.bodyJSON(matjson::makeObject({
//                 {"account_id", accID},
//                 {"user_id", userID},
...
//         }, "LT Login Task 1/2")
//         .chain([](web::WebResponse* res) -> LoginTask {
//             if (res->ok()){
