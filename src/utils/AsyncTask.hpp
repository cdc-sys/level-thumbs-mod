...
// This file contains full copy of the geode::Task class, except the chain method has ability to map progress
// TODO: remove this when PR is merged:
// https://github.com/geode-sdk/geode/pull/1481
...
class [[nodiscard]] CustomTask final  {
public:
...
private:
    ...
    CustomTask(std::shared_ptr<Handle> handle) : m_handle(handle) {}
...
public:
    ...
    CustomTask() : m_handle(nullptr) {}

    CustomTask(CustomTask const& other) : m_handle(other.m_handle) {}
    CustomTask(CustomTask&& other) : m_handle(std::move(other.m_handle)) {}
    CustomTask& operator=(CustomTask const& other) {
        m_handle = other.m_handle;
        return *this;
    }
    CustomTask& operator=(CustomTask&& other) {
        m_handle = std::move(other.m_handle);
        return *this;
    }

    bool operator==(CustomTask const& other) {
        return m_handle == other.m_handle;
    }
    bool operator!=(CustomTask const& other) const {
        return m_handle != other.m_handle;
    }
    bool operator<(CustomTask const& other) const {
        return m_handle < other.m_handle;
     ...
     */
    void shallowCancel() {
        CustomTask::cancel(m_handle, true);
    }
     ...
     */
    static CustomTask cancelled(std::string_view name = "<Cancelled Task>") {
        auto task = CustomTask(Handle::create(name));
        CustomTask::cancel(task.m_handle);
        return task;
    }
    /**
     ...
     * @param name The name of the Task; used for debugging
     */
    static CustomTask immediate(Type value, std::string_view name = "<Immediate Task>") {
        auto task = CustomTask(Handle::create(name));
        CustomTask::finish(task.m_handle, std::move(value));
        return task;
    }
    /**
     ...
     * @param name The name of the Task; used for debugging
     */
    static CustomTask run(Run&& body, std::string_view name = "<Task>") {
        auto task = CustomTask(Handle::create(name));
        std::thread([handle = std::weak_ptr(task.m_handle), name = std::string(name), body = std::move(body)] {
            ...
            auto result = body(
                [handle](P progress) {
                    CustomTask::progress(handle.lock(), std::move(progress));
                },
                [handle]() -> bool {
            ...
            if (result.isCancelled()) {
                CustomTask::cancel(handle.lock());
            }
            else {
                CustomTask::finish(handle.lock(), std::move(*std::move(result).getValue()));
            }
        }).detach();
        return task;
    }
     ...
     * @param name The name of the Task; used for debugging
     */
    static CustomTask runWithCallback(RunWithCallback&& body, std::string_view name = "<Callback Task>") {
        auto task = CustomTask(Handle::create(name));
        std::thread([handle = std::weak_ptr(task.m_handle), name = std::string(name), body = std::move(body)] {
            utils::thread::setName(fmt::format("Task '{}'", name));
            body(
     ...
     */
    static std::tuple<CustomTask, PostResult, PostProgress, HasBeenCancelled> spawn(std::string_view name = "<Task>") {
    ...
    static Task<std::vector<Type*>, std::monostate> all(std::vector<Task<Type, NP>>&& tasks, std::string_view name = "<Multiple Tasks>") {
        using AllTask = CustomTask<std::vector<Type*>, std::monostate>;
    ...
    auto map(ResultMapper&& resultMapper, ProgressMapper&& progressMapper, OnCancelled&& onCancelled, std::string_view name = "<Mapping Task>") const {
        ...
        CustomTask<T2, P2> task = CustomTask<T2, P2>::Handle::create(fmt::format("{} <= {}", name, m_handle->m_name));
        ...
        // If the current task is finished, immediately map the value and post that
            ...
            CustomTask<T2, P2>::finish(task.m_handle, std::move(resultMapper(&*m_handle->m_resultValue)));
        }
        // Otherwise start listening and waiting for the current task to finish
        else {
            task.m_handle->m_extraData = std::make_unique<typename CustomTask<T2, P2>::Handle::ExtraData>(
                static_cast<void*>(new EventListener<CustomTask>(
                    [
                        ...
                        if (auto v = event->getValue()) {
                            CustomTask<T2, P2>::finish(handle.lock(), std::move(resultMapper(v)));
                        }
                        else if (auto p = event->getProgress()) {
                            CustomTask<T2, P2>::progress(handle.lock(), std::move(progressMapper(p)));
                        }
                        else if (event->isCancelled()) {
                            onCancelled();
                            CustomTask<T2, P2>::cancel(handle.lock());
                        }
                    },
                    *this
                )),
                +[](void* ptr) {
                    delete static_cast<EventListener<CustomTask>*>(ptr);
                },
                +[](void* ptr) {
                    // Cancel the mapped task too
                    static_cast<EventListener<CustomTask>*>(ptr)->getFilter().cancel();
                }
            );
        }
        return task;
    ...
    void listen(OnResult&& onResult, OnProgress&& onProgress, OnCancelled&& onCancelled) const {
        ...
        auto* listener = new EventListener<CustomTask>(*this);
        listener->bind([
...
static_assert(is_filter<CustomTask<int>>, "The Task class must be a valid event filter!");