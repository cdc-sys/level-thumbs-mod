...
//
//     auto timestamp = std::move(res).unwrap();
//
//     std::chrono::sys_time<std::chrono::microseconds> utc;
//     {
//         std::istringstream iss(timestamp);
//         iss >> parse("%Y-%m-%dT%H:%M:%S.%f", utc);
//         if (iss.fail()) return timestamp;
//     }
//
//     auto secs = time_point_cast<std::chrono::seconds>(utc);
//     auto tm = geode::localtime(std::chrono::system_clock::to_time_t(secs));
//
